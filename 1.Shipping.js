...
function get_orders_by_date(order_date, status) {
  const orders = data.orders.filter(order => {
    const orderDate = order.orderDate.split('T')[0];
    const isStatusMatch = status ? order.status === status : true;
    return orderDate === order_date && isStatusMatch;
  }).map(order => {
...
const orderDate = '2024-07-29';
const status = 'Processing';
const orders = get_orders_by_date(orderDate, status);
